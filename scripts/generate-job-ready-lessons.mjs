/**
 * One-off generator for power-bi, python, and statistics lesson data files.
 */
...
// ─── POWER BI ───────────────────────────────────────────────────────────────
const powerBiLessons = [
  {
    title: 'Power BI Desktop Interface',
    partial: makePartial({
      title: 'Power BI Desktop Interface',
      subjectLabel: 'Power BI',
      topic: 'the Power BI Desktop workspace',
      ...
      stakeholderRole: 'Sales Director',
      stakeholderQ: 'Can you rebuild the West region revenue view in Power BI so we can slice by product category?',
      ...
      componentBreakdown: [
        { part: 'Report view', explanation: 'Canvas for charts, tables, slicers, and KPI cards.' },
        { part: 'Data view', explanation: 'Spreadsheet-like preview of each table in the model.' },
        { part: 'Model view', explanation: 'Diagram of tables and relationships — the heart of accurate DAX.' },
        { part: 'Fields pane', explanation: 'Lists tables, columns, measures, and hierarchies available to visuals.' },
        { part: 'Visualizations pane', explanation: 'Pick chart types and format properties for the selected visual.' },
      ],
      ...
      guidedExample: {
        ...
        steps: ['Get Data → Text/CSV → select northstar_orders.csv', 'Open Data view and verify order_date is a date type', 'Switch to Model view and note single-table model before adding customers'],
      },
      tags: ['interface', 'desktop', 'workspace'],
      projectName: 'Power BI Business Report Project',
    }),
    quiz: [
      { question: 'Which Power BI Desktop view shows table relationships as a diagram?', options: ['Report view', 'Data view', 'Model view', 'Service view'], correctIndex: 2, explanation: 'Model view displays tables and relationship lines between them.' },
      { question: 'Where do you drag fields to create a bar chart?', options: ['Filters pane only', 'Report canvas with Visualizations pane', 'Power Query Editor only', 'Excel Online'], correctIndex: 1, explanation: 'Report view combines the canvas, Fields pane, and Visualizations pane.' },
      { question: 'What file extension does a Power BI Desktop report use?', options: ['.xlsx', '.pbix', '.twbx', '.csv'], correctIndex: 1, explanation: 'Power BI Desktop saves work as .pbix files.' },
    ],
    exerciseInstructions: 'List the three main Power BI Desktop views and one task you perform in each for a Northstar sales report.',
    exerciseHint: 'Think: build charts, inspect rows, connect tables.',
    exerciseAnswer: 'Report view — design visuals; Data view — inspect table values and types; Model view — create and verify relationships between orders and customers.',
    exerciseExplanation: 'Each view maps to a phase of analytics: visualize, inspect, and model.',
  },
  {
    ...
    partial: makePartial({
      title: 'Importing Data',
      subjectLabel: 'Power BI',
      topic: 'importing Northstar data sources',
      ...
      stakeholderQ: 'Can you combine our campaign spend spreadsheet with order data to see ROI by channel?',
      ...
      componentBreakdown: [
        { part: 'Get Data', explanation: 'Launches connectors for files, databases, and online services.' },
        { part: 'Navigator', explanation: 'Preview tables and sheets before loading.' },
        { part: 'Import mode', explanation: 'Copies data into the model — best for most Northstar CSV/Excel sizes.' },
        { part: 'DirectQuery', explanation: 'Queries live at source — use when data is huge or must be real-time.' },
        { part: 'Transform Data', explanation: 'Opens Power Query Editor before load.' },
      ],
      ...
      guidedExample: {
        ...
        steps: ['Get Data → Text/CSV', 'Select northstar_customers.csv', 'Click Load', 'Data view → customers → check row count and region column'],
      },
      tags: ['import', 'get-data', 'connectors'],
      projectName: 'Power BI Business Report Project',
    }),
    quiz: [
      { question: 'Which ribbon button starts importing data?', options: ['Publish', 'Get Data', 'New Measure', 'Optimize'], correctIndex: 1, explanation: 'Get Data opens the connector gallery.' },
      { question: 'Import mode stores data where?', options: ['Only in Excel', 'Inside the Power BI model in memory/on disk', 'Only on the SQL server', 'In Power Query only'], correctIndex: 1, explanation: 'Import copies data into the .pbix model for fast queries.' },
      { question: 'When should you click Transform Data instead of Load?', options: ['When data needs cleaning before modeling', 'When publishing to web', 'When deleting a visual', 'When writing DAX'], correctIndex: 0, explanation: 'Transform Data opens Power Query for shaping steps first.' },
    ],
    exerciseInstructions: 'Northstar has orders in SQL and campaigns in Excel. Describe your import sequence and which mode you would use for each.',
    exerciseHint: 'Consider size and refresh needs for a mid-market retailer.',
    exerciseAnswer: 'Import orders from SQL (Import mode for typical Northstar volume) and marketing_campaigns from Excel via Get Data → Excel; use Transform Data on Excel if headers need fixing, then Load both into the model.',
    exerciseExplanation: 'Import mode suits most Northstar datasets; Transform handles Excel cleanup before load.',
  },
  {
    ...
    partial: makePartial({
      title: 'Power Query Data Cleaning',
      subjectLabel: 'Power BI',
      topic: 'Power Query transformations',
      ...
      stakeholderQ: 'Why does total revenue in Power BI not match our finance spreadsheet?',
      ...
      componentBreakdown: [
        { part: 'Applied Steps', explanation: 'Ordered list of transformations you can edit or delete.' },
        { part: 'Changed Type', explanation: 'Ensures dates and currency calculate correctly in DAX.' },
        { part: 'Remove Duplicates', explanation: 'Keeps one row per key such as order_id.' },
        { part: 'Split Column', explanation: 'Separates combined fields like full_name into first and last.' },
        { part: 'Close & Apply', explanation: 'Runs the query and updates the model.' },
      ],
      ...
      guidedExample: {
        ...
        steps: ['Transform Data on orders query', 'Remove top rows if export has title line', 'Use First Row as Headers', 'Change order_date to Date', 'Remove Duplicates on order_id', 'Close & Apply'],
      },
      tags: ['power-query', 'cleaning', 'transform'],
      projectName: 'Power BI Business Report Project',
    }),
    quiz: [
      { question: 'Where are Power Query steps recorded?', options: ['Applied Steps pane', 'Filters pane', 'DAX formula bar', 'Publish dialog'], correctIndex: 0, explanation: 'Applied Steps shows the transformation pipeline.' },
      { question: 'Why change column types in Power Query?', options: ['So DAX and visuals treat dates and numbers correctly', 'To change chart colors', 'To add slicers automatically', 'To publish faster'], correctIndex: 0, explanation: 'Correct types prevent sort errors and wrong aggregations.' },
      { question: 'What happens to Power Query steps on refresh?', options: ['They re-run on new source data', 'They are deleted', 'They only run once', 'They move to Excel'], correctIndex: 0, explanation: 'Refresh replays all steps against the latest source file.' },
    ],
    exerciseInstructions: 'A Northstar products export has trailing spaces in category names and mixed-case regions. List three Power Query steps to fix it.',
    exerciseHint: 'Think trim, replace, and type consistency.',
    exerciseAnswer: 'Trim category column; Replace Values or capitalize regions to standardize; Change Type on list_price to decimal number.',
    exerciseExplanation: 'Text cleanup and type fixes in Power Query ensure accurate filters and sums.',
  },
  {
    title: 'Relationships and Data Modeling',
    partial: makePartial({
      title: 'Relationships and Data Modeling',
      subjectLabel: 'Power BI',
      topic: 'table relationships in the data model',
      ...
      stakeholderQ: 'When I filter to Midwest, why do product counts look the same as national?',
      ...
      componentBreakdown: [
        { part: 'Cardinality', explanation: 'One-to-many is standard: one customer, many orders.' },
        { part: 'Cross-filter direction', explanation: 'Single direction filters facts when slicing dimensions.' },
        { part: 'Active relationship', explanation: 'Only one active path between two tables unless using USERELATIONSHIP in DAX.' },
        { part: 'Primary key', explanation: 'Unique identifier on the one side — customer_id, product_id.' },
        { part: 'Foreign key', explanation: 'Repeating key on the many side linking back to dimension.' },
      ],
      ...
      guidedExample: {
        ...
        steps: ['Model view → relate customers to orders on customer_id', 'Create matrix: customers[region] rows, count of orders[order_id]', 'Add slicer on region → confirm counts change'],
      },
      tags: ['relationships', 'modeling', 'keys'],
      projectName: 'Power BI Business Report Project',
    }),
    quiz: [
      { question: 'In a one-to-many relationship, where does the "one" side usually live?', options: ['Dimension table like customers', 'Fact table like order line items', 'Calculated table only', 'Slicer table'], correctIndex: 0, explanation: 'Dimensions (customers, products) are on the one side; facts on the many side.' },
      { question: 'What happens if orders and customers are not related?', options: ['Filters may not propagate and totals can be wrong', 'Power BI auto-fixes on publish', 'Only colors break', 'Nothing — relationships are optional'], correctIndex: 0, explanation: 'Without relationships, cross-table filtering and DAX context break down.' },
      { question: 'Which column should link orders to customers?', options: ['customer_id', 'customer email only', 'order_date', 'product_name'], correctIndex: 0, explanation: 'Stable numeric keys like customer_id are correct join keys.' },
    ],
    exerciseInstructions: 'Draw the relationship chain from customers to revenue at order line level for Northstar.',
    exerciseHint: 'customers → orders → order_items → products.',
    exerciseAnswer: 'customers (1) → (*) orders on customer_id; orders (1) → (*) order_items on order_id; products (1) → (*) order_items on product_id — revenue = SUM(order_items[quantity] * order_items[unit_price]).',
    exerciseExplanation: 'Star-style paths from dimensions to facts enable correct filtered revenue.',
  },
  {
    title: 'Star Schema',
    partial: makePartial({
      ...
      stakeholderQ: 'I want one dashboard where region, product category, and campaign channel all filter the same revenue number.',
      ...
      componentBreakdown: [
        { part: 'Fact table', explanation: 'Numeric events — sales lines, clicks, tickets — at detailed grain.' },
        { part: 'Dimension tables', explanation: 'Descriptive attributes — region, category, campaign name.' },
        { part: 'Date table', explanation: 'Continuous dates for YoY and month-to-date logic.' },
        { part: 'Single direction', explanation: 'Filters flow from dimensions to facts in classic star design.' },
      ],
      ...
      guidedExample: {
        ...
        steps: ['Split wide export into products dimension in Power Query', 'Keep line-level rows in order_items', 'Relate products to order_items', 'Build revenue measure on order_items'],
      },
      tags: ['star-schema', 'fact', 'dimension'],
      projectName: 'Power BI Business Report Project',
    }),
    quiz: [
      { question: 'In a retail star schema, which is typically the fact table?', options: ['order_items at line grain', 'customers', 'products only', 'Region lookup'], correctIndex: 0, explanation: 'Line-level sales transactions are the fact table.' },
      { question: 'Why use a dedicated date table?', options: ['Enables consistent time intelligence and fiscal calendars', 'Required to import CSV', 'Replaces relationships', 'Only for maps'], correctIndex: 0, explanation: 'A marked date table supports DAX time functions reliably.' },
      { question: 'Star schema filters generally flow:', options: ['From dimensions to facts', 'From facts to dimensions only', 'Randomly', 'Not at all'], correctIndex: 0, explanation: 'Single-direction star filtering keeps semantics clear.' },
    ],
    exerciseInstructions: 'Name the fact table and three dimensions you would use for Northstar campaign ROI analysis.',
    exerciseHint: 'Think line-level sales and descriptive tables.',
    exerciseAnswer: 'Fact: order_items (or orders+items); Dimensions: customers, products, marketing_campaigns, and Date.',
    exerciseExplanation: 'Campaign ROI joins spend from marketing to revenue through orders at consistent grain.',
  },
  {
    ...
    partial: makePartial({
      title: 'DAX Fundamentals',
      subjectLabel: 'Power BI',
      topic: 'DAX formulas and evaluation context',
      ...
      stakeholderRole: 'CEO',
      stakeholderQ: 'What was total revenue last month compared to the same month last year?',
      ...
      componentBreakdown: [
        { part: 'Measure', explanation: 'Calculated in context of visuals and filters — preferred for KPIs.' },
        { part: 'Calculated column', explanation: 'Computed row-by-row; increases model size — use sparingly.' },
        { part: 'SUM / SUMX', explanation: 'SUM adds a column; SUMX iterates rows for line-level math.' },
        { part: 'DIVIDE', explanation: 'Safe division with alternate result on zero denominator.' },
      ],
      ...
      guidedExample: {
        ...
        steps: ['New Measure on order_items for revenue with SUMX', 'New Measure on orders for DISTINCTCOUNT order_id', 'Add both to a card visual and test with region slicer'],
      },
      tags: ['dax', 'measures', 'fundamentals'],
      projectName: 'Power BI Business Report Project',
    }),
    quiz: [
      { question: 'What recalculates when a slicer changes?', options: ['Measure', 'Imported column only', 'Power Query step', 'Theme color'], correctIndex: 0, explanation: 'Measures evaluate in the current filter context.' },
      { question: 'Which function iterates rows for quantity * price?', options: ['SUMX', 'COUNT', 'LEFT', 'RELATED'], correctIndex: 0, explanation: 'SUMX multiplies per row then sums — ideal for line revenue.' },
      { question: 'Safe division in DAX uses:', options: ['DIVIDE', 'UNION', 'CALENDAR', 'USERELATIONSHIP'], correctIndex: 0, explanation: 'DIVIDE handles zero denominators gracefully.' },
    ],
    exerciseInstructions: 'Write a DAX measure for Average Order Value at Northstar using Total Revenue and Order Count.',
    exerciseHint: 'Use DIVIDE.',
    exerciseAnswer: 'Average Order Value = DIVIDE([Total Revenue], [Order Count])',
    exerciseExplanation: 'DIVIDE avoids errors when order count is zero for a filter selection.',
  },
  {
    ...
    partial: makePartial({
      title: 'Measures and Calculated Columns',
      subjectLabel: 'Power BI',
      topic: 'choosing between measures and calculated columns',
      ...
      stakeholderRole: 'Sales Manager',
      stakeholderQ: 'Can we show revenue per order and also total revenue by region on the same page?',
      ...
      componentBreakdown: [
        { part: 'Measure', explanation: 'Dynamic aggregation — shows in Values well in visuals.' },
        { part: 'Calculated column', explanation: 'Static per row — appears in Rows/Columns like imported fields.' },
        { part: 'Implicit measure', explanation: 'Dragging numeric column to Values — often wrong for line facts.' },
        { part: 'Format string', explanation: 'Currency and percentage display for stakeholder readability.' },
      ],
      ...
      guidedExample: {
        ...
        steps: ['Create Total Revenue measure', 'Add Line Amount calculated column', 'Table visual with product_name, Line Amount, and Total Revenue in card filtered same way'],
      },
      tags: ['measures', 'calculated-columns', 'dax'],
      projectName: 'Power BI Business Report Project',
    }),
    quiz: [
      { question: 'Which updates when a user clicks a slicer?', options: ['Measure', 'Calculated column values at refresh only', 'SQL source table', 'Theme'], correctIndex: 0, explanation: 'Measures re-evaluate with filter context; columns are fixed at refresh.' },
      { question: 'Where should Total Revenue live?', options: ['As a measure on the fact table', 'As a text column on customers', 'Only in Excel', 'In a slicer'], correctIndex: 0, explanation: 'Revenue KPIs belong in measures on fact tables.' },
      { question: 'Calculated columns increase:', options: ['Model size and refresh time', 'Only chart colors', 'SQL Server CPU only', 'Nothing'], correctIndex: 0, explanation: 'Columns materialize values for every row in the model.' },
    ],
    exerciseInstructions: 'When would you create a calculated column for Northstar order size bucket (Small/Medium/Large) instead of a measure?',
    exerciseHint: 'Slicers and row-level labels.',
    exerciseAnswer: 'When you need a row-level category on each order for use as a slicer or axis label — e.g., Order Size Bucket column based on line quantity thresholds.',
    exerciseExplanation: 'Buckets at row grain are column work; totals of revenue still use measures.',
  },
  {
    ...
    partial: makePartial({
      title: 'CALCULATE and Filter Context',
      subjectLabel: 'Power BI',
      topic: 'CALCULATE and filter context manipulation',
      ...
      stakeholderRole: 'Regional Director',
      stakeholderQ: 'What percent of company revenue comes from the Midwest?',
      ...
      componentBreakdown: [
        { part: 'CALCULATE', explanation: 'Evaluates expression under modified filter context.' },
        { part: 'Filter argument', explanation: 'Column = value, or FILTER table expression.' },
        { part: 'ALL', explanation: 'Removes filters from named columns or tables.' },
        { part: 'ALLSELECTED', explanation: 'Respects visual totals context — common in % of parent.' },
      ],
      ...
      guidedExample: {
        ...
        steps: ['Write Midwest Revenue with CALCULATE filter on region', 'Write % of Total with ALL on region in denominator', 'Matrix: region rows, Total Revenue and % measures'],
      },
      tags: ['calculate', 'filter-context', 'dax'],
      projectName: 'Power BI Business Report Project',
    }),
    quiz: [
      { question: 'CALCULATE primarily modifies:', options: ['Filter context', 'Chart title', 'CSV path', 'Theme font'], correctIndex: 0, explanation: 'CALCULATE changes which rows are included in evaluation.' },
      { question: 'ALL(customers[region]) in a denominator typically:', options: ['Removes region filter for total comparison', 'Deletes data', 'Adds a slicer', 'Imports Excel'], correctIndex: 0, explanation: 'ALL clears filters on that column for the calculation inside CALCULATE.' },
      { question: 'Which wraps an existing measure with a filter?', options: ['CALCULATE([Total Revenue], ...)', 'SUM(customers)', 'GETDATA()', 'PUBLISH()'], correctIndex: 0, explanation: 'CALCULATE takes a measure and optional filter arguments.' },
    ],
    exerciseInstructions: 'Write DAX for Northstar shipped-order revenue only (orders[status] = "Shipped").',
    exerciseHint: 'CALCULATE + filter on status.',
    exerciseAnswer: 'Shipped Revenue = CALCULATE([Total Revenue], orders[status] = "Shipped")',
    exerciseExplanation: 'CALCULATE applies the status filter while computing Total Revenue.',
  },
  {
    title: 'Visualizations and Chart Selection',
    partial: makePartial({
      title: 'Visualizations and Chart Selection',
      subjectLabel: 'Power BI',
      topic: 'choosing the right visual for Northstar metrics',
      ...
      stakeholderRole: 'Marketing Director',
      stakeholderQ: 'Show me monthly paid search spend vs revenue so I can see if spend spikes paid off.',
      ...
      componentBreakdown: [
        { part: 'Bar/column chart', explanation: 'Compare discrete categories — regions, categories, campaigns.' },
        { part: 'Line chart', explanation: 'Show trends over continuous time.' },
        { part: 'Matrix', explanation: 'Two-dimensional breakdown with subtotals.' },
        { part: 'Card/KPI', explanation: 'Single headline number with optional trend indicator.' },
        { part: 'Table', explanation: 'Row detail for export and audit.' },
      ],
      ...
      guidedExample: {
        ...
        steps: ['Bar chart with category and Total Revenue', 'Line chart with Date month and Total Revenue', 'Add date slicer and sync both visuals'],
      },
      tags: ['visualizations', 'charts', 'report-design'],
      projectName: 'Power BI Business Report Project',
    }),
    quiz: [
      { question: 'Best visual for revenue trend over 12 months?', options: ['Line chart', 'Pie chart', 'Gauge only', 'Map only'], correctIndex: 0, explanation: 'Line charts show continuous time trends clearly.' },
      { question: 'Best visual to compare revenue across four regions?', options: ['Clustered bar chart', 'Single KPI card', 'Text box', 'Slicer only'], correctIndex: 0, explanation: 'Bars compare categorical values side by side.' },
      { question: 'Matrix visual is ideal for:', options: ['Two-dimensional breakdowns like region × category', 'Geographic lat/long only', 'Writing DAX', 'Importing CSV'], correctIndex: 0, explanation: 'Matrices show row and column groups with aggregations.' },
    ],
    exerciseInstructions: 'Choose visuals for: (a) MTD revenue single number, (b) top 5 product categories, (c) monthly orders trend.',
    exerciseHint: 'Card, bar, line.',
    exerciseAnswer: '(a) KPI card; (b) sorted bar chart with top N filter; (c) line chart with Date on axis and Order Count as values.',
    exerciseExplanation: 'Each question maps to a visual type optimized for that data shape.',
  },
  {
    title: 'Slicers, Filters, and Interactivity',
    partial: makePartial({
      title: 'Slicers, Filters, and Interactivity',
      subjectLabel: 'Power BI',
      topic: 'slicers and report-level filters',
      ...
      stakeholderRole: 'Support Lead',
      stakeholderQ: 'Let me filter tickets by region and month but hide cancelled orders from everyone.',
      ...
      componentBreakdown: [
        { part: 'Slicer', explanation: 'Visible filter UI for report consumers.' },
        { part: 'Report-level filter', explanation: 'Applies globally — use for data quality rules.' },
        { part: 'Page-level filter', explanation: 'Scopes one page — e.g., Marketing page only digital channel.' },
        { part: 'Visual-level filter', explanation: 'Top N, specific chart exclusions.' },
        { part: 'Sync slicers', explanation: 'Same slicer selection across multiple report pages.' },
      ],
      ...
      guidedExample: {
        ...
        steps: ['Insert region slicer on page 1', 'Sync to page 2', 'Add report filter on status', 'Verify totals match finance subset'],
      },
      tags: ['slicers', 'filters', 'interactivity'],
      projectName: 'Power BI Business Report Project',
    }),
    quiz: [
      { question: 'Report-level filters apply to:', options: ['All pages in the report', 'One visual only', 'Power Query only', 'Excel source'], correctIndex: 0, explanation: 'Report filters are global unless overridden at lower levels.' },
      { question: 'Sync slicers ensures:', options: ['Same slicer selection across selected pages', 'Automatic DAX writing', 'SQL refresh', 'Map colors'], correctIndex: 0, explanation: 'Sync keeps filter context consistent multi-page.' },
      { question: 'Visual-level Top N filter affects:', options: ['Only that visual', 'Entire organization', 'Database server', 'All subjects'], correctIndex: 0, explanation: 'Visual filters scope to the selected visual.' },
    ],
    exerciseInstructions: 'Design filter strategy for Northstar report: exclude test customers, show region slicer, keep one page digital-only.',
    exerciseHint: 'Report filter, slicer, page filter.',
    exerciseAnswer: 'Report-level filter customer segment <> Test; synced region slicer on all pages; page-level filter channel = Digital on marketing page only.',
    exerciseExplanation: 'Layered filters separate global rules, user exploration, and page-specific scope.',
  },
  {
    title: 'Dashboard Design and Publishing',
    partial: makePartial({
      title: 'Dashboard Design and Publishing',
      subjectLabel: 'Power BI',
      topic: 'dashboard layout and publishing to Power BI Service',
      ...
      stakeholderRole: 'COO',
      stakeholderQ: 'Publish the regional sales dashboard and make sure it refreshes every morning before our 9 AM standup.',
      ...
      componentBreakdown: [
        { part: 'Visual hierarchy', explanation: 'Most important KPIs top-left where eyes land first.' },
        { part: 'Publish', explanation: 'Uploads report and dataset to Power BI Service.' },
        { part: 'Scheduled refresh', explanation: 'Keeps Northstar data current automatically.' },
        { part: 'Gateway', explanation: 'Connects Service to on-premises SQL if needed.' },
        { part: 'Workspace roles', explanation: 'Admin/Member/Viewer control edit vs view access.' },
      ],
      ...
      guidedExample: {
        ...
        steps: ['Publish to Analytics workspace', 'Configure credentials for SQL and Excel on gateway', 'Set 6 AM refresh and confirm success email', 'Share with Sales Leadership group'],
      },
      tags: ['publishing', 'dashboard-design', 'service'],
      projectName: 'Power BI Business Report Project',
    }),
    quiz: [
      { question: 'Publishing saves the report to:', options: ['Power BI Service workspace', 'Only your laptop', 'Excel Online only', 'Power Query'], correctIndex: 0, explanation: 'Publish uploads to the cloud Service workspace.' },
      { question: 'Scheduled refresh requires:', options: ['Valid data source credentials and often a gateway for on-prem', 'Only a pie chart', 'Deleting relationships', 'Export to PDF'], correctIndex: 0, explanation: 'Refresh needs authenticated access to sources.' },
      { question: 'Good dashboard design places key KPIs:', options: ['Prominently at top with clear hierarchy', 'Hidden on last page', 'Only in footnotes', 'In Power Query editor'], correctIndex: 0, explanation: 'Visual hierarchy guides attention to primary metrics.' },
    ],
    exerciseInstructions: 'List three pre-publish checks for a Northstar Power BI report.',
    exerciseHint: 'Data, model, audience.',
    exerciseAnswer: 'Verify relationships and measure totals against finance; confirm refresh credentials/gateway; review layout, definitions, and workspace permissions.',
    exerciseExplanation: 'Validation prevents wrong numbers and failed overnight refresh.',
  },
  {
    ...
    partial: makePartial({
      title: 'Power BI Business Report Project',
      subjectLabel: 'Power BI',
      topic: 'capstone Northstar sales and marketing report',
      ...
      stakeholderQ: 'Deliver one published report where I can filter region and month and see revenue, orders, spend, and open tickets together.',
      ...
      componentBreakdown: [
        { part: 'Data pipeline', explanation: 'Import + Power Query documented steps.' },
        { part: 'Model', explanation: 'Star schema with Date table marked.' },
        { part: 'DAX layer', explanation: 'Reusable measures with clear names.' },
        { part: 'Report UX', explanation: 'Slicers, hierarchy, mobile layout.' },
        { part: 'Deployment', explanation: 'Publish, refresh, share, document.' },
      ],
      ...
      guidedExample: {
        ...
        steps: ['Card: Total Revenue, Orders, Marketing Spend, ROI', 'Matrix: region × category', 'Line: monthly revenue trend', 'Sync date and region slicers to detail pages'],
      },
      tags: ['project', 'capstone', 'portfolio'],
      projectName: 'Power BI Business Report Project',
    }),
    quiz: [
      { question: 'First step in the capstone after gathering requirements?', options: ['Import and assess data quality in Power Query', 'Publish empty report', 'Delete relationships', 'Only add pie charts'], correctIndex: 0, explanation: 'Solid data foundation precedes modeling and visuals.' },
      { question: 'Star schema for Northstar revenue uses fact table:', options: ['order_items at line grain', 'Only customers', 'Slicer table only', 'Theme colors'], correctIndex: 0, explanation: 'Line-level facts support product and revenue detail.' },
      { question: 'Before stakeholder demo you should:', options: ['Validate totals and test slicer combinations', 'Disable refresh', 'Remove all measures', 'Hide every visual'], correctIndex: 0, explanation: 'QA against known totals builds credibility.' },
    ],
    exerciseInstructions: 'Outline four pages for the Northstar capstone report and one primary question each page answers.',
    exerciseHint: 'Executive, sales, marketing, support angles.',
    exerciseAnswer: 'Executive — Are we hitting revenue targets? Sales — Which categories drive regional growth? Marketing — Which channels deliver ROI? Support — Where are ticket backlogs by region?',
    exerciseExplanation: 'Page-level focus keeps dashboards scannable and stakeholder-aligned.',
...
writeSubjectFile({
  subjectId: 'power-bi',
  exportPrefix: 'powerBi',
  projectName: 'Power BI Business Report Project',
  subjectLabel: 'Power BI',
  lessons: powerBiLessons,
});

console.log('Power BI done. Run extended generator for python and statistics.');
